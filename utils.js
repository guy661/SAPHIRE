...
class Logger {
    ...
    _log(level, message) {
        ...
        console.log(`${this.color}[${this.prefix}] ${this.emoji} [${timestamp}] ${message}${COLORS.reset}`);
    }
    ...
    warn(message) {
        console.warn(`${this.color}[${this.prefix}] ${this.emoji} [${new Date().toLocaleTimeString()}] [WARN] ${message}${COLORS.reset}`);
    }

    error(message, errorObj = null) {
        const errorMessage = errorObj ? `${message} | Stack: ${errorObj.stack}` : message;
        console.error(`${this.color}[${this.prefix}] ${this.emoji} [${new Date().toLocaleTimeString()}] [ERROR] ${errorMessage}${COLORS.reset}`);
    }
